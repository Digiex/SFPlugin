...
public class SFPlugin extends JavaPlugin {
    ...
    public static ItemStack[] stringToItemStack(String invString) {
        ...
        for (int i = 0; i < firstSplit.length - 1; i++) {
            if (!firstSplit[(i + 1)].equals("null")) {
                ...
                itemStack[i] = new ItemStack(Integer.valueOf(secondSplit[0]).intValue(),
                        Integer.valueOf(secondSplit[1]).intValue(), Short.valueOf(secondSplit[2]).shortValue());
    ...
    public SFInventory getSFInventory(GameMode gameMode, String playerName) {
        ...
        SFInventory inv = (SFInventory) getDatabase().find(SFInventory.class).where().eq("gameMode", gm).ieq("playerName", playerName).findUnique();
        return inv;
    }
